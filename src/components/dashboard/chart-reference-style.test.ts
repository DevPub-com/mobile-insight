...
describe("reference dashboard chart style", () => {
  ...
  it("matches the reference legend order, palette, solid lines, and filled circle markers", () => {
    ...
    expect(source).toContain('lineStyle: { color, width: 2.25, type: "solid" as const }');
    expect(source).toContain('itemStyle: { color, borderColor: color, borderWidth: 0 }');
    ...
    expect(ratingSource).toContain('itemStyle: { color, borderColor: color, borderWidth: 0 }');
    expect(activeUserSource).toContain('["Total", "total", "#8993A7", 2.4]');
    expect(activeUserSource).toContain('["Android", "android", "#22A447", 1.8]');
  ...
  it("colors delta states and review stars with semantic tokens", () => {
    expect(globalStyles).toMatch(/\.mi-change--increase\s*\{[^}]*color:\s*var\(--mi-success\);/s);
    expect(globalStyles).toMatch(/\.mi-change--decrease\s*\{[^}]*color:\s*var\(--mi-danger\);/s);
    expect(globalStyles).toMatch(/\.mi-change--flat\s*\{[^}]*color:\s*var\(--mi-warning\);/s);
    expect(globalStyles).toMatch(/\.mi-review-meta strong\s*\{[^}]*color:\s*var\(--mi-warning\);/s);
    ...
    expect(globalStyles).toMatch(/\.mi-metric-icon--red\s*\{[^}]*color:\s*var\(--mi-danger\);[^}]*background:\s*var\(--mi-danger-soft\);/s);
  });

  it("uses neutral gray before and brand blue after in release impact charts", () => {
    expect(impactTrendSource).toContain('lineStyle: { color: "#8993A7", width: 2 }');
    expect(impactTrendSource).toContain('lineStyle: { color: "#4C73EC", width: 2.5 }');
  });
  ...
  it("removes KPI sparkline markers, smooths the curve, and uses a one-pixel line", () => {
    ...
    expect(sparklineSource).toContain("lineStyle: { color, width: 1 }");
    expect(sparklineSource).not.toContain("itemStyle:");
    expect(sparklineSource).toContain('color: `${color}77`');
