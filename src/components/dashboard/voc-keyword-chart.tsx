...
export function VocKeywordChart({ groups, onSelect }: {
  ...
  return <div className="mi-voc-visual">
    ...
    <div className="mi-voc-chart-heading"><strong>많이 언급된 소분류 TOP 7</strong><span>언급 리뷰 수</span></div>
    {top.length ? <ol className="mi-voc-chart" aria-label="키워드별 감정 분포">
      {top.map((row) => <li key={row.key}>
        <button type="button" className="mi-voc-chart-row" onClick={() => onSelect({ label: row.label, key: row.key, grade: filter })}
          aria-label={`${row.label}: ${row.total}개 리뷰 · ${grades.map((grade) => `${keywordGradeLabel[grade]} ${row[grade]}`).join(", ")} · 관련 리뷰 보기`}>
          <span className="mi-voc-chart-label" title={(JSON.parse(row.key) as string[]).filter(Boolean).map(value => `#${value}`).join(" ")}>{row.label}</span>
    ...
    {chips.length > 0 && <details className="mi-voc-keyword-details">
      ...
      <div className="mi-voc-keyword-buttons">{chips.map(({ label, key, grade, count }) =>
        <button type="button" key={`${grade}:${key}`} className={`mi-ai-topic-chip is-${grade}`} onClick={() => onSelect({ label, key, grade })}
